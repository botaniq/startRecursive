...
function startRecursive() {
  ...
  return async function recursive() {
    return await Promise.all(users.splice(0, 10).map(el => processUser(el.id)))
      .then(res => res.reduce((sum, el) => sum + el))
      .then(res => sum += res)
      .then(() => users.length > 0 ? recursive() : sum)
  }();
